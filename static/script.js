...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document
    ...
    .addEventListener("click", () => showPage("enhancement"));
  document
    .getElementById("classificationBtn")
    .addEventListener("click", () => showPage("classification"));
  document
    .getElementById("backToHomeFromEnhancement")
    .addEventListener("click", () => showPage("home"));
  document
    .getElementById("backToHomeFromClassification")
    .addEventListener("click", () => showPage("home"));
  ...
  const classifyOriginalImage = document.getElementById(
    "classifyOriginalImage"
  );
  const classifyOriginalPlaceholder = document.getElementById(
    "classifyOriginalPlaceholder"
  );
  ...
  classifyImageUpload.addEventListener("change", function () {
    if (this.files.length > 0) {
      ...
      fetch("/upload", {
        ...
        .then((data) => {
          // Update original image source with server response
          classifyOriginalImage.src =
            data.original + "?t=" + new Date().getTime();
        });
    }
  });
  ...
  classifyBtn.addEventListener("click", function () {
    ...
    fetch("/process", {
          ...
          html += `
                    <div class="mb-3">
                        <div class="flex justify-between text-sm mb-1">
                            <span class="font-medium">${pred[0].replace(
                              /_/g,
                              " "
                            )}</span>
      ...
      .catch((err) => {
        ...
        classificationResults.innerHTML =
          '<div class="text-red-500">Error during classification</div>';
      });
  });
});
